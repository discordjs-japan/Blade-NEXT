...
module.exports = class extends Language {
  ...
  constructor(...args) {
    super(...args)
    this.language = {
      DEFAULT: (key) => `${key} はja-JPではまだ翻訳されていません。`,
      ...
      RESOLVER_MINMAX_MAX: (name, max, suffix) => `${name} は ${max}${suffix} よち小さい必要があります。`,
      REACTIONHANDLER_PROMPT: 'どのページにジャンプしますか？',
      COMMANDMESSAGE_MISSING: '入力の終了後に一つ以上の必須の引数が不足しています。',
      COMMANDMESSAGE_MISSING_REQUIRED: (name) => `${name} は引数が必要です。`,
      COMMANDMESSAGE_MISSING_OPTIONALS: (possibles) => `必要なオプションが不足しています: (${possibles})`,
      COMMANDMESSAGE_NOMATCH: (possibles) => `あなたの選択肢はいずれの可能性とも一致しませんでした: (${possibles})`,
      MONITOR_COMMAND_HANDLER_REPROMPT: (tag, error, time) => `${tag} | **${error}** | **${time}** 秒の間に有効な引数を入力してください。**"ABORT"** と打つとこのプロンプトは中止されます。`, // eslint-disable-line max-len
      MONITOR_COMMAND_HANDLER_REPEATING_REPROMPT: (tag, name, time) => `${tag} | **${name}** は繰り返しの引数です | **${time}** 秒の間に追加の引数を入力してください。**"CANCEL"** と打つとこのプロンプトは中止されます。`, // eslint-disable-line max-len
      ...
      INHIBITOR_MISSING_BOT_PERMS: (missing) => `不十分なアクセス許可、不足しています: **${missing}**`,
      ...
      INHIBITOR_RUNIN_NONE: (name) => `${name}コマンドは、どのチャネルでも実行出来るようになっていません。`,
      COMMAND_BLACKLIST_DESCRIPTION: 'ボットからのユーザーやサーバーをブラックリストまたはブラックリストに登録します。',
      ...
      COMMAND_EVAL_EXTENDEDHELP: [
        'evalコマンドはコードを評価し、そこから発生したエラーは全て処理されます。',
      ...
      COMMAND_TRANSFER_ERROR: '❌ その言語ファイルは既に選択されているか、言語ファイル存在しません。',
      ...
      COMMAND_LOAD: (time, type, name) => `✅ 読み込みました。${type}: ${name}. (所要時間: ${time})`,
      COMMAND_LOAD_FAIL: 'ファイルが存在しないか、ファイルのロード中にエラーが発生しました。コンソールを確認してください。',
      COMMAND_LOAD_ERROR: (type, name, error) => `❌ 読み込みに失敗しました。${type}: ${name}. Reason:${util.codeBlock('js', error)}`,
      ...
      COMMAND_INVITE: () => [
        ...
        util.codeBlock('', [
          ...
          'すべての権限がすべてのギルドにとって正しいわけではないので、ボックスのチェックを外すことを恐れないでください。',
      ...
      COMMAND_INFO: [
        ...
        '• 🎚🎛 自分のフィールドで拡張できるクライアント/ギルド/ユーザ設定',
        '• 💬 自動化されたパラメータ解決機能とオンザフライでコマンドをロード/リロードする機能を備えたカスタマイズ可能なコマンドシステム',
        '• 👀 "モニター", メッセージの送信や編集の閲覧が可能です。（swear フィルター、スパム保護など。）',
        '• ⛔ "インヒビター", これは、適用する条件（許可、ブラックリストなど）に基づいてコマンドが実行されるのを防ぐことができます。',
        '• 🗄 "プロバイダ", あなたの選択したデータベースの使用を簡素化します',
        '• ✅ "ファイナライザー", コマンドの成功後に実行が行われます。（ログ、統計の収集、応答のクリーンアップなど。）',
        '• ➕ "拡張性", 既存のDiscord.jsクラスまたはKlasaクラスにメソッド、ゲッター/セッター、または静的プロパティを受動的に追加する',
        '• 🌐 "言語", ボットの応答をローカライズすることが可能です。',
        '• ⏲ "タスク", 将来実行されるようにスケジュールされ、必要に応じて繰り返し実行されます',
        '',
        'すべてのユーザーに対応できる100％カスタマイズ可能なフレームワークになることを願っています。利用可能な場合は、頻繁な更新とバグ修正を行います。',
        'もし興味がある場合は、https://klasa.js.org をご覧ください。'
      ],
      COMMAND_INFO_DESCRIPTION: 'このボットに関するいくつかの情報を表示します。',
      ...
      COMMAND_ENABLE: (type, name) => `+ 有効になりました。${type}: ${name}`,
      COMMAND_ENABLE_DESCRIPTION: 'コマンド/インヒビター/モニター/ファイナライザーを再度有効にするか、一時的に有効にします。再起動するとデフォルトの状態に戻ります。',
      COMMAND_DISABLE: (type, name) => `+ 無効になりました。${type}: ${name}`,
      COMMAND_DISABLE_DESCRIPTION: 'コマンド/インヒビター/モニター/ファイナライザー/イベントを再度無効にするか、一時的に無効にします。再起動するとデフォルトの状態に戻ります。',
      ...
      COMMAND_DISCORDSTATS_GREED: '全サーバーは正常です。',
      COMMAND_DISCORDSTATS_RED: 'サーバーが不安定な可能性があります。',
      COMMAND_DISCORDSTATS_LAST_MAINTENANCE: '❯ 最後に行われたメンテナンス',
      ...
      COMMAND_USERINFO_DESCRIPTION: 'ユーザーの情報を取得して表示します。',
      COMMAND_USERINFO_ONLINE: '💚 オンライン',
      COMMAND_USERINFO_IDLE: '💛 退席中',
      COMMAND_USERINFO_DND: '❤ 起こさないで',
      COMMAND_USERINFO_OFFLINE: '💔 オフライン',
      COMMAND_USERINFO_NAME: '❯ 名前',
      COMMAND_USERINFO_ID: '❯ ID',
      COMMAND_USERINFO_DISCORDJOINDATE: '❯ Discordを始めた日',
      COMMAND_USERINFO_SERVERJOINDATE: '❯ サーバーに参加した日',
      COMMAND_USERINFO_STATUS: '❯ ステータス',
      COMMAND_USERINFO_PLAYING: '❯ 遊んでるゲーム',
      COMMAND_USERINFO_HIGHESTROLE: '❯ 最も高い役職',
      COMMAND_USERINFO_HOISTROLE: '❯ オンラインリストに役職の名前が表示されるか',
      COMMAND_ROLEINFO_DESCRIPTION: '役職の詳しい情報を表示します。',
      COMMAND_ROLEINFO_ADMINISTRATOR: '管理者',
      COMMAND_ROLEINFO_VIEW_AUDIT_LOG: 'サーバーログの表示',
      COMMAND_ROLEINFO_MANAGE_GUILD: 'サーバー管理',
      COMMAND_ROLEINFO_MANAGE_ROLES: '役職の管理',
      COMMAND_ROLEINFO_MANAGE_CHANNELS: 'チャンネル管理',
      COMMAND_ROLEINFO_KICK_MEMBERS: 'メンバーをキック',
      COMMAND_ROLEINFO_BAN_MEMBERS: 'メンバーをBAN',
      COMMAND_ROLEINFO_CREATE_INSTANT_INVITE: '招待リンクの作成',
      COMMAND_ROLEINFO_CHANGE_NICKNAME: 'ニックネーム変更',
      COMMAND_ROLEINFO_MANAGE_NICKNAMES: 'ニックネームの管理',
      COMMAND_ROLEINFO_MANAGE_EMOJIS: '絵文字の管理',
      COMMAND_ROLEINFO_MANAGE_WEBHOOKS: 'Webhookの管理',
      COMMAND_ROLEINFO_VIEW_CHANNEL: 'テキストチャンネルの閲覧・ボイスチャンネルの一覧表示',
      COMMAND_ROLEINFO_SEND_MESSAGES: 'メッセージ送信',
      COMMAND_ROLEINFO_SEND_TTS_MESSAGES: 'TTSメッセージ送信',
      COMMAND_ROLEINFO_MANAGE_MESSAGES: 'メッセージの管理',
      COMMAND_ROLEINFO_EMBED_LINKS: '埋め込みリンク',
      COMMAND_ROLEINFO_ATTACH_FILES: 'ファイルを',
      COMMAND_ROLEINFO_READ_MESSAGE_HISTORY: 'メッセージの履歴を読む',
      COMMAND_ROLEINFO_MENTION_EVERYONE: '全員宛メンション',
      COMMAND_ROLEINFO_USE_EXTERNAL_EMOJIS: '外部絵文字の使用',
      COMMAND_ROLEINFO_ADD_REACTIONS: 'リアクションの追加',
      COMMAND_ROLEINFO_CONNECT: '接続',
      COMMAND_ROLEINFO_SPEAK: '発言',
      COMMAND_ROLEINFO_MUTE_MEMBERS: 'メンバーをミュート',
      COMMAND_ROLEINFO_DEAFEN_MEMBERS: 'メンバーのスピーカーをミュート',
      COMMAND_ROLEINFO_MOVE_MEMBERS: 'メンバーを移動',
      COMMAND_ROLEINFO_USE_VAD: '音声検出を使用',
      COMMAND_ROLEINFO_NAME: '❯ 役職の名前',
      COMMAND_ROLEINFO_ID: '❯ ID',
      COMMAND_ROLEINFO_COLOR: '❯ 色',
      COMMAND_ROLEINFO_CREATION_DATE: '❯ 作成日',
      COMMAND_ROLEINFO_HOISTED: '❯ オンラインリストに役職の名前が表示されるか',
      COMMAND_ROLEINFO_MENTIONABLE: 'この役職へのメンションが出来るか',
      COMMAND_ROLEINFO_PERMISSIONS: '権限一覧',
      YES: 'はい',
      NO: 'いいえ',
      NONE: '無い',
